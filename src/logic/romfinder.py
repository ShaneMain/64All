...
def _prompt_user_to_select_file(file_list):
    ...
    QMessageBox.information(None, "Select ROM File", "Multiple valid ROM files found. Please select one.")
    ...
    dialog.setDirectory(os.getcwd())  # Set the directory to the current working directory

    # Set options to ensure only one file can be selected
    dialog.setOption(QFileDialog.Option.DontUseNativeDialog)  # Optional: Use custom dialog for consistency
...
def _prompt_user_for_file():
    ...
    # Display the information message box
    QMessageBox.information(None, "Select ROM File", "Please select a valid Mario 64 .z64 ROM file.")
...
class N64RomValidator:
    ...
    def check_rom_files(self):
        ...
        for file_name in rom_files:
            ...
            if file_hash == self.KNOWN_HASHES.get(file_name):
    ...
    def find_or_select_file(self):
        ...
        if valid_files:
            if len(valid_files) == 1:
                ...
                key, self.file_path = valid_files[0]
                final_file_dir = os.path.dirname(self.file_path)
                final_file_region = key
                print(f"Found valid N64 ROM: {self.file_path}")
            else:
                # Multiple valid files found, prompt the user to select one
                selected_file_path = _prompt_user_to_select_file([f for _, f in valid_files])
            ...
            while True:
                ...
                if self.file_path:
                    ...
                    if key:
                        ...
                        QMessageBox.critical(None, "Invalid ROM",
        ...
        return final_file_region, final_file_dir
